...
import { requireAuthenticatedPageContext } from "@/lib/auth/server";
import { createRepositories } from "@/lib/db";
import { getD1Client } from "@/lib/db/client";

export default async function AccountPage() {
  const { user, session } = await requireAuthenticatedPageContext();
  const repositories = createRepositories(await getD1Client());
  const licenses = await repositories.licenses.listByUserId(user.id);
        ...
        <div className="portal-grid">
          ...
          <article className="portal-panel">
            ...
            <dl className="portal-details">
              ...
              <div><dt>Expires</dt><dd>{new Date(session.expires_at).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}</dd></div>
            </dl>
          </article>
        </div>
